...
The `.prose-section` class in `css/style.css` is a lightweight utility for text-heavy content blocks — useful wherever you need a clean column of copy with properly spaced paragraphs and styled subheadings. Apply it to the inner container `<div>` alongside `container container-narrow`:

```html
<section class="fade-in" style="padding: 80px 0;">
  <div class="container container-narrow prose-section">
...
`.prose-section` only sets `h3` font-size/margin and `p` margin-bottom — nothing else is affected. Currently used on `index.html` for the additional copy block between the Services grid and the Contact section.
